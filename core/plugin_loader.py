...
class PluginRegistry:
    ...
    def get_tool_declarations(self) -> list[dict]:
        ...
        for name, rec in self._plugins.items():
            if get_plugin_enabled(name) and self._is_available(rec):
                decls.append(
                    {
                        "name": rec.name,
    ...
    def run(self, name: str, parameters: dict, player=None, session_memory=None) -> str:
        ...
        if not self._is_available(rec):
            return f"The '{name}' plugin is not available in the current configuration."
        try:
    ...
    def list_for_ui(self) -> list[dict]:
        ...
        for rec in self._all_records:
            available = self._is_available(rec) if rec.valid else False
            out.append(
                {
                    "name": rec.name,
                    "description": rec.description,
    ...
    def _is_available(self, rec: PluginRecord) -> bool:
        if rec.available is None:
            return True
        try:
            return bool(rec.available())
        except Exception as exc:
            self._logger(f"Plugin '{rec.name}' availability check failed: {exc}")
            return False


def _call_run(run_fn, parameters, player, session_memory):
    """Invoke run() passing only kwargs declared by the plugin."""
...
def _validate(module, filename: str) -> PluginRecord:
    """Return a PluginRecord; validation errors never escape discovery."""
    ...
    if not isinstance(name, str) or not _NAME_RE.match(name):
        return PluginRecord(
            ...
            file=filename,
            error=(
                "PLUGIN['name'] missing or not a valid identifier "
    ...
    available_fn = getattr(module, "is_available", None)
    if available_fn is not None and not callable(available_fn):
        return PluginRecord(
            name=name,
            file=filename,
            error="Optional is_available must be callable when defined.",
        )
...
def discover_plugins(
    ...
    """Discover plugins without allowing one bad file to abort the scan."""
    ...
    logger(
        f"Plugin discovery complete: {len(valid)} active, "
