...
# Qt-free module inside the ui package: load it from its file so the
# dependency-free CI legs (no PyQt6) can exercise it without importing
# the ui package __init__.
_ROOT = Path(__file__).resolve().parent.parent
_spec = importlib.util.spec_from_file_location(
    "antonella_voice_feedback", _ROOT / "ui" / "voice_feedback.py"
)
_module = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = _module  # required by dataclass string annotations
_spec.loader.exec_module(_module)

VoiceFeedback = _module.VoiceFeedback
cancelled_feedback = _module.cancelled_feedback
execution_result_to_voice_feedback = _module.execution_result_to_voice_feedback
recovery_feedback = _module.recovery_feedback

sys.path.insert(0, str(_ROOT))
from core.execution_result import ExecutionResult  # noqa: E402  (stdlib-only module)
...
class VoiceFeedbackMappingTests(unittest.TestCase):
    ...
    def test_failure_is_human_and_short(self):
        ...
        # The technical error must not leak into the spoken phrase.
        self.assertNotIn("Playwright", feedback.phrase_pt)

    def test_waiting_approval_never_approves(self):
        feedback = execution_result_to_voice_feedback(
    ...
    def test_cancelled_is_its_own_category(self):
        ...
        self.assertNotIn("falhou", feedback.phrase_pt.lower())
    ...
    def test_property_no_success_speech_before_verified_success(self):
        # Across every boolean combination, only the fully verified one
        # (without approval pending and without error) may announce success.
        for ok, delivered, verified, has_error, requires_approval in itertools.product(
            (False, True), repeat=5
        ):
            with self.subTest(
                ok=ok, delivered=delivered, verified=verified,
                has_error=has_error, requires_approval=requires_approval,
            ):
                feedback = execution_result_to_voice_feedback(
                    _result(
                        ok=ok,
                ...
                if feedback.category == "verified_success":
                    self.assertTrue(ok and delivered and verified and not has_error and not requires_approval)
    ...
    def test_feedback_is_frozen_and_categories_are_valid(self):
        ...
        self.assertIn(feedback.category, _module.CATEGORIES)
